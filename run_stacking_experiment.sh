...
export SAM2_LOG_LEVEL="ERROR"

# ===========================================================================
# Train stacked model + evaluate 3 models:
#   1) base             → single slices (its natural input)
#   2) stacked_causal   → 8-frame clips (its natural input)
#   3) stacked_bidir    → 8-frame clips (same checkpoint, better inference)
# ===========================================================================
...
if [ $# -lt 1 ]; then
  ...
  echo
  echo "Example:"
  echo "  $0 stack_comparison_e10_b20"
  echo "  $0 stack_comparison_e10_b20 --max-cases-per-dataset 5000"
  echo
  echo "Environment overrides:"
  echo "  PYTHON=<path>                 default: python"
  echo "  DATASETS_ROOT=<path>          default: data/new_datasets"
  echo "  BASE_CHECKPOINT=<path>        default: checkpoints/sam2.1_hiera_tiny.pt"
  echo "  INFER_CONFIG=<path>           default: sam2/configs/sam2.1_hiera_t512.yaml"
  echo "  IMAGE_CHANNEL_INDEX=<int>     default: 0"
  echo "  NUM_GPUS=<int>                default: 1"
  echo "  WINDOW_SIZE=<int>             default: 8"
  echo "  WINDOW_STRIDE=<int>           default: 8"
  exit 1
fi
...
DATASETS_ROOT="${DATASETS_ROOT:-data/new_datasets}"
BASE_CHECKPOINT="${BASE_CHECKPOINT:-checkpoints/sam2.1_hiera_tiny.pt}"
INFER_CONFIG="${INFER_CONFIG:-sam2/configs/sam2.1_hiera_t512.yaml}"
IMAGE_CHANNEL_INDEX="${IMAGE_CHANNEL_INDEX:-0}"
NUM_GPUS="${NUM_GPUS:-1}"
WINDOW_SIZE="${WINDOW_SIZE:-8}"
WINDOW_STRIDE="${WINDOW_STRIDE:-8}"

SPLITS_ROOT="${DATASETS_ROOT}/_splits/${EXPERIMENT_NAME}"
EXP_LOG="exp_log/${EXPERIMENT_NAME}"
EVAL_ROOT="${EXP_LOG}/eval"
...
if ! "$PYTHON" -c "import sam2" 2>/dev/null; then
  echo "ERROR: 'sam2' not importable. Activate medsam2 environment first."
  exit 1
fi
...
echo
echo "========================================================"
echo " Step 1/5  Preparing experiment: ${EXPERIMENT_NAME}"
echo "========================================================"

"$PYTHON" scripts/prepare_stacking_experiment.py \
...
if [ ! -f "$META_JSON" ]; then
  echo "ERROR: experiment.json not found at $META_JSON"
  exit 1
fi

STACK_CONFIG_NAME="$("$PYTHON" -c "import json; print(json.load(open('$META_JSON'))['stack_config_name'])")"
STACK_OUTPUT="$("$PYTHON" -c "import json; print(json.load(open('$META_JSON'))['stack_output'])")"
STACK_CKPT="${STACK_OUTPUT}/checkpoints/checkpoint.pt"

# ---------------------------------------------------------------------------
# Step 2 – Train stacked model
# ---------------------------------------------------------------------------
echo
echo "========================================================"
echo " Step 2/5  Training STACKED model (num_frames=${WINDOW_SIZE})"
echo "========================================================"

"$PYTHON" -m training.train \
  -c "$STACK_CONFIG_NAME" \
  --output-path "$STACK_OUTPUT" \
  --use-cluster 0 \
  --num-gpus "$NUM_GPUS" \
  --num-nodes 1

if [ ! -f "$STACK_CKPT" ]; then
  echo "ERROR: Stacked checkpoint not found at $STACK_CKPT"
  exit 1
fi
echo "Stacked training complete: $STACK_CKPT"
...
echo
echo "========================================================"
echo " Step 3/5  Evaluating 3 models"
echo "========================================================"
...
for ds in sorted(by_ds.keys()):
    ...
    for tag in ("stacked_causal", "stacked_bidir"):
        if tag not in by_ds[ds]: continue
        r = by_ds[ds][tag]
        dd = r["dice"] - base["dice"]; di = r["iou"] - base["iou"]
...
result = {
    "description": "Base on 1-slice, Stacked on 8-frame clips",
    "model_info": {tag: {"label": l, "input": i} for tag, (l, i) in MODEL_INFO.items()},
    "per_model": rows,
